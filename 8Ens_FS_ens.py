...
#average_votes = np.zeros((Sample_size,n_splits))
final_votes = np.zeros((Sample_size,n_splits))
...
for train_index, test_index in kf.split(X_all_window):
    ...
    for i in range(len(X.columns)):
        ...
        y_pred_matrix_whole[train_index,i] = y_pred_train
        y_pred_matrix_whole[test_index,i] = y_pred            
        evaluation_matrix_train[i,:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_train, y_pred_train,labels=[0,1]))
        evaluation_matrix_train[i,-1,fold_num-1] = roc_auc_score(y_train, y_pred_train)
        evaluation_matrix_test[i,:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_test, y_pred,labels=[0,1]))
        evaluation_matrix_test[i,-1,fold_num-1] = roc_auc_score(y_test, y_pred)
    ...
    final_votes[train_index,fold_num-1] = y_pred_train
    final_votes[test_index,fold_num-1] = y_pred
    evaluation_metric_test[:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_test, final_votes[test_index,fold_num-1],labels=[0,1]))
    evaluation_metric_test[-1,fold_num-1] = roc_auc_score(y_test, final_votes[test_index,fold_num-1])
    evaluation_metric_train[:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_train, final_votes[train_index,fold_num-1],labels=[0,1]))
    evaluation_metric_train[-1,fold_num-1] = roc_auc_score(y_train, final_votes[train_index,fold_num-1])
    ...
    print('Ens:'+str(Ensemble1)+' '+str(Ensemble2)+" (#test fold:"+str(fold_num)+")")
    print(result_df)
    current_time = dt.datetime.now().strftime("%H.%M.%S")
    with pd.ExcelWriter(r'C:\Users\momas\OneDrive\Desktop\Thesis\Thesis\CODES\Revised Codes\Results\M8\8.xlsx', engine="openpyxl",mode='a',if_sheet_exists='overlay') as writer:
        result_df.to_excel(writer, sheet_name=str(current_time)+' '+str(fold_num),startcol=3*(fold_num-1),index=False)
 ...
 #   evaluation_final_test = confusion_metrics(confusion_matrix(y_test, y_pred,labels=[0,1]))
  ...
  #  result_df = pd.DataFrame({'metric':evaluation_measures,'value over test':evaluation_final_test,'value over train':evaluation_final_train})
   ...
   # print("#fold for test:"+str(fold_num))
    ...
    plt.plot(t, final_votes[:,fold_num-1], label='Ensenbles: '+str(Ensemble1)+' '+str(Ensemble2))
    ...
    plt.title('Prediction of Ensemble '+ str(Ensemble2)+' with FS(Ens1:'+ str(Ensemble1)+' #test fold:'+str(fold_num)+')',fontsize=22)
    ...
    #plt.annotate('Test Set', xy=(0.2, 0.9), xytext=((fold_num-.5)/n_splits, .1), xycoords='axes fraction', 
     #       fontsize=20, ha='center', va='center',
      #      bbox=dict(boxstyle='round', fc='skyblue'),
        ...
        #    )
    #y_final[train_index] , y_final[test_index] = y_pred_train, y_pred
    plt.plot(t_np[y_auxfill2_idx], final_votes[y_auxfill2_idx,fold_num-1], label='Ens2: '+str(Ensemble2))
             #, linestyle='dashed', linewidth = 3,
         #marker='o', markerfacecolor='blue', 
         #        markersize=12)
        #plt.plot(t, final_votes[:,mod], label='Vote'+str(model_list[mod]))
    ...
    # giving a title to my graphco
    plt.title('Prediction of Double Ensembles with a Feature Selection Step(Ensembles:'+ str(Ensemble1)+''+ str(Ensemble2)+',#test fold:'+str(fold_num)+')',fontsize=22)
    plt.gca().spines["top"].set_alpha(.0)
    plt.gca().spines["bottom"].set_alpha(.3)
    plt.gca().spines["right"].set_alpha(.0)
    plt.gca().spines["left"].set_alpha(.3)
