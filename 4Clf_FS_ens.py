...
for train_index, test_index in kf.split(X_all_window):
    ...
    ### Feature Selection:   
    for counter in range(len(model_list)):
        sorted_index_array = np.argsort(evaluation_matrix_train[:,counter,Metr,fold_num-1],axis=0)
        indices[:,counter,fold_num-1] = sorted_index_array[-n:]
    ...
    #average_votes[:,mod] = np.mean(y_pred_matrix_whole[:,mod*len(X.columns):(mod+1)*len(X.columns)],axis=1)
        fs_index = counter*len(X.columns)+indices[:,counter,fold_num-1]
        ...
        print('clf1:'+str(model_list[counter])+" (#test fold:"+str(fold_num)+")")
        print(result_df)
        current_time = dt.datetime.now().strftime("%H.%M.%S")
        with pd.ExcelWriter(r'C:\Users\momas\OneDrive\Desktop\Thesis\Thesis\CODES\Revised Codes\Results\M4\4.xlsx', engine="openpyxl",mode='a',if_sheet_exists='overlay') as writer:
            result_df.to_excel(writer, sheet_name=str(current_time)+' '+str(fold_num-1),startcol=3*counter,index=False)
  ...
  #  result_df = pd.DataFrame({'metric':evaluation_measures,'value over test':evaluation_final_test,'value over train':evaluation_final_train})
   ...
   # print("#fold for test:"+str(fold_num))
    ...
    for mod in range(len(model_list)):
        ...
        plt.plot(t, final_votes[:,mod,fold_num-1], label='clf1: '+str(model_list[mod]))
    ...
    plt.title('Prediction of Ensemble '+ str(Ensemble2)+' with FS(various Clf1) (#test fold:'+str(fold_num)+')',fontsize=22)
    ...
    for mod in range(len(model_list)):
        #y_final[train_index] , y_final[test_index] = y_pred_train, y_pred
        plt.plot(t_np[y_auxfill2_idx], final_votes[y_auxfill2_idx,mod,fold_num-1], label='clf1: '+str(model_list[mod]))
             #, linestyle='dashed', linewidth = 3,
         #marker='o', markerfacecolor='blue', 
         #        markersize=12)
        #plt.plot(t, final_votes[:,mod], label='Vote'+str(model_list[mod]))
