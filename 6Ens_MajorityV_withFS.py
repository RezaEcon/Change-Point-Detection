...
#average_votes[:,mod] = np.mean(y_pred_matrix_whole[:,mod*len(X.columns):(mod+1)*len(X.columns)],axis=1)
    ...
    average_votes[:,fold_num-1] = np.mean(y_pred_matrix_whole[:,fs_index],axis=1)
    final_votes[:,fold_num-1] = np.round(average_votes[:,fold_num-1])
    ...
    print('MetaClf:'+str(model_list[Ensemble1])+" (#test fold:"+str(fold_num)+")")
    print(result_df)
    current_time = dt.datetime.now().strftime("%H.%M.%S")
    with pd.ExcelWriter(r'C:\Users\momas\OneDrive\Desktop\Thesis\Thesis\CODES\Revised Codes\Results\M6\6.xlsx', engine="openpyxl",mode='a',if_sheet_exists='overlay') as writer:
        result_df.to_excel(writer, sheet_name=str(current_time),startcol=3*(fold_num-1),index=False)
 ...
 #   evaluation_final_test = confusion_metrics(confusion_matrix(y_test, y_pred,labels=[0,1]))
  ...
  #  result_df = pd.DataFrame({'metric':evaluation_measures,'value over test':evaluation_final_test,'value over train':evaluation_final_train})
   ...
   # print("#fold for test:"+str(fold_num))
    ...
    plt.plot(t, average_votes[:,fold_num-1], label='avr '+str(model_list[Ensemble1]))
    ...
    #plt.annotate('Test Set', xy=(0.2, 0.9), xytext=((fold_num-.5)/n_splits, .1), xycoords='axes fraction', 
     #       fontsize=20, ha='center', va='center',
      #      bbox=dict(boxstyle='round', fc='skyblue'),
        ...
        #    )
        #y_final[train_index] , y_final[test_index] = y_pred_train, y_pred
    plt.plot(t_np[y_auxfill2_idx], average_votes[y_auxfill2_idx,fold_num-1], label='avr '+str(model_list[fold_num-1]))
             #, linestyle='dashed', linewidth = 3,
         #marker='o', markerfacecolor='blue', 
         #        markersize=12)
        #plt.plot(t, final_votes[:,mod], label='Vote'+str(model_list[mod]))
    ...
    # giving a title to my graphco
    plt.title('Prediction of Ensemble with a Majority Vote(over best features)'+' (#test fold:'+str(fold_num)+')',fontsize=22)
    plt.gca().spines["top"].set_alpha(.0)
    plt.gca().spines["bottom"].set_alpha(.3)
    plt.gca().spines["right"].set_alpha(.0)
    plt.gca().spines["left"].set_alpha(.3)
