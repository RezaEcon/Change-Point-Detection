#### Ensemble 1: Logistic0, SVM,Decision Tree,Random Forest,Naive Bayes5
  ## EasyEns6,RUSBoost,BalancedBagging,BalancedRandomForestClassifier
Ensemble1 = 0
#### Ensemble 2: Logistic, SVM, KNN,Decision Tree,Random Forest,Naive Bayes
...
for train_index, test_index in kf.split(X_all_window):
    ...
    for i in range(len(X.columns)):
        ...
        for model in model_pipeline:
            ...
            if recurrent !=0:
                ...
                y_pred = meta_clf.predict(X_test2)
                y_pred_train = meta_clf.predict(X_train2)
    ...
    for mod in range(len(model_list)):
        evaluation_metric_test[mod,:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_test, final_votes[test_index,mod],labels=[0,1]))
        evaluation_metric_test[mod,-1,fold_num-1] = roc_auc_score(y_test, final_votes[test_index,mod])
        evaluation_metric_train[mod,:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_train, final_votes[train_index,mod],labels=[0,1]))
        evaluation_metric_train[mod,-1,fold_num-1] = roc_auc_score(y_train, final_votes[train_index,mod])
        result_df = pd.DataFrame({'metric':evaluation_measures,'value over test':evaluation_metric_test[mod,:,fold_num-1],'value over train':evaluation_metric_train[mod,:,fold_num-1]})
        print('clf:'+str(model_list[mod])+" (#test fold:"+str(fold_num)+")")
        print(result_df)
        current_time = dt.datetime.now().strftime("%H.%M.%S")
        with pd.ExcelWriter(r'C:\Users\momas\OneDrive\Desktop\Thesis\Thesis\CODES\Revised Codes\Results\M1\1.xlsx', engine="openpyxl",mode='a',if_sheet_exists='overlay') as writer:
            result_df.to_excel(writer, sheet_name=str(current_time),startcol=3*mod,index=False)
  ...
  #  result_df = pd.DataFrame({'metric':evaluation_measures,'value over test':evaluation_final_test,'value over train':evaluation_final_train})
   ...
   # print("#fold for test:"+str(fold_num))
    ...
    for mod in range(len(model_list)):
        ...
        plt.plot(t, average_votes[:,mod], label='avr '+str(model_list[mod]))
    ...
    plt.title('Prediction of Classifiers with a Majority Vote(over all features)'+' (#test fold:'+str(fold_num)+')',fontsize=22)
    ...
    for mod in range(len(model_list)):
        #y_final[train_index] , y_final[test_index] = y_pred_train, y_pred
        plt.plot(t_np[y_auxfill2_idx], average_votes[y_auxfill2_idx,mod], label='avr '+str(model_list[mod]))
             #, linestyle='dashed', linewidth = 3,
         #marker='o', markerfacecolor='blue', 
         #        markersize=12)
        #plt.plot(t, final_votes[:,mod], label='Vote'+str(model_list[mod]))
