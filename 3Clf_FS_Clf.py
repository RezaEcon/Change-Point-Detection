...
for train_index, test_index in kf.split(X_all_window):
    ...
    #average_votes[:,mod] = np.mean(y_pred_matrix_whole[:,mod*len(X.columns):(mod+1)*len(X.columns)],axis=1)
        ...
        with pd.ExcelWriter(r'C:\Users\momas\OneDrive\Desktop\Thesis\Thesis\CODES\Revised Codes\Results\M3\3.xlsx', engine="openpyxl",mode='a',if_sheet_exists='overlay') as writer:
