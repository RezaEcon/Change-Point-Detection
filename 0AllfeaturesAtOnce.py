...
##### K-Fold Splits
n_splits = 5
##### Feature Selection threshold and metric
perc = .1
Metr = 7
...
y_test_size = math.ceil(test_percent*Sample_size)
y_train_size = Sample_size - y_test_size
...
model_pipeline = []
model_pipeline.append(LogisticRegression(solver='lbfgs',max_iter=100000,class_weight = w))
...
#model_pipeline.append(AdaBoostClassifier(n_estimators=100, random_state=0))
...
if Ensemble1 == 0:
    meta_clf = StackingClassifier(stack_method='predict_proba',estimators=estimators_step1, final_estimator=LogisticRegression(solver='lbfgs',max_iter=50000,class_weight=w))
elif Ensemble1 == 1:
 ...
 #   meta_clf = StackingClassifier(estimators=estimators_step1, final_estimator=KNeighborsClassifier())
    #meta_clf = StackingClassifier(estimators=estimators_step1, final_estimator=AdaBoostClassifier(n_estimators=100, random_state=0))
#elif Ensemble1 == 3:
...
#acc_matrix_step1 = np.zeros((len(X.columns)-1,len(model_list)))
#auc_matrix_step1 = np.zeros((len(X.columns)-1,len(model_list)))
#score_matrix_step2 = np.zeros((len(X.columns)-1,1))
#g_mean_list_step2 = np.zeros((len(X.columns)-1,1))
#cm_matrix_step1 = np.zeros( ( (len(X.columns)-1)*len(model_list),2,2) )
#g_mean_matrix = np.zeros((len(X.columns)-1,len(model_list)))
...
for i in range(len(X.columns)):
    ...
    M_all = (pd.DataFrame(M_all).fillna(0)).to_numpy()  
...
#y_pred_matrix_whole_folds = np.zeros((Sample_size,len(X.columns)*len(model_list),n_splits))
y_pred_matrix_whole_folds = np.zeros((Sample_size,n_splits))
average_votes = np.zeros((Sample_size,len(model_list)))
final_votes = np.zeros((Sample_size,len(model_list)))
evaluation_measures = ["accuracy","misclassification","sensitivity","specificity","g_mean","precision","f1","f3","auc"]
evaluation_metric_test = np.zeros((num_metrics,n_splits))
evaluation_metric_train = np.zeros((num_metrics,n_splits))
#evaluation_matrix_train = np.zeros((len(X.columns),len(model_list),num_metrics,n_splits))
#evaluation_matrix_test = np.zeros((len(X.columns),len(model_list),num_metrics,n_splits))
n = math.ceil(perc*(len(X.columns)))
indices = np.zeros((n,len(model_list),n_splits),dtype=int)
kf = KFold(n_splits=n_splits, random_state=None, shuffle=False)
for train_index, test_index in kf.split(X_all_window_Copy):
    fold_num = 1+round(test_index[0]*n_splits/Sample_size)
    #if fold_num != n_splits:
     #   continue
      #  print('hello'+str(fold_num))
    ##standardization:
    transformer= StandardScaler().fit(X_all_window_Copy[train_index,:])  
    ...
    y_pred_matrix_whole = np.zeros(Sample_size)
    ...
    y_pred_matrix_whole[train_index] = y_pred_train
    y_pred_matrix_whole[test_index] = y_pred
    yPR_tr = meta_clf.predict_proba(X_train)[:, 1]
    yPR_tst = meta_clf.predict_proba(X_test)[:, 1]
    ### adding previous period's estimated probability (predict_proba) as a feature to next window
    if recurrent !=0:
        ...
        X_all_window2 = np.concatenate((X_all_window,aa_pd),axis=1)
        ...
        y_pred = meta_clf.predict(X_test)
        y_pred_train = meta_clf.predict(X_train)
        y_pred_matrix_whole2 = np.zeros(Sample_size)
        y_pred_matrix_whole2[train_index] = y_pred_train
        y_pred_matrix_whole2[test_index] = y_pred
        yPR_tr2 = meta_clf.predict_proba(X_train)[:, 1]
        yPR_tst2 = meta_clf.predict_proba(X_test)[:, 1]
    
    #evaluation_matrix_train[i,counter,:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_train, y_pred_train,labels=[0,1]))
    #evaluation_matrix_train[i,counter,-1,fold_num-1] = roc_auc_score(y_train, y_pred_train)
    #evaluation_matrix_test[i,counter,:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_test, y_pred,labels=[0,1]))
    #evaluation_matrix_test[i,counter,-1,fold_num-1] = roc_auc_score(y_test, y_pred)
    y_pred_matrix_whole_folds[:,fold_num-1] = y_pred_matrix_whole
    ### Feature Selection:
    #for mod in range(len(model_list)):
     #   sorted_index_array = np.argsort(evaluation_matrix_train[:,mod,Metr,fold_num-1],axis=0)
      #  indices[:,mod,fold_num-1] = sorted_index_array[-n:]
    ...
    #average_votes[:,mod] = np.mean(y_pred_matrix_whole[:,mod*len(X.columns):(mod+1)*len(X.columns)],axis=1)
        #fs_index = mod*len(X.columns)+indices[:,mod,fold_num-1]
    #average_votes[:,mod] = np.mean(y_pred_matrix_whole[:,fs_index],axis=1)
    #final_votes = np.round(average_votes)
    
    #for mod in range(len(model_list)):
    evaluation_metric_test[:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_test, y_pred,labels=[0,1]))
    evaluation_metric_test[-1,fold_num-1] = roc_auc_score(y_test, y_pred)
    #evaluation_metric_test[-1,fold_num-1] = roc_auc_score(y_test, yPR_tst)
    evaluation_metric_train[:-1,fold_num-1] = confusion_metrics(confusion_matrix(y_train, y_pred_train,labels=[0,1]))
    evaluation_metric_train[-1,fold_num-1] = roc_auc_score(y_train, y_pred_train)
    #evaluation_metric_train[-1,fold_num-1] = roc_auc_score(y_train, yPR_tr)
    result_df = pd.DataFrame({'metric':evaluation_measures,'value over test':evaluation_metric_test[:,fold_num-1],'value over train':evaluation_metric_train[:,fold_num-1]})
    print('MetaClf:'+str(Ensemble1)+" (#test fold:"+str(fold_num)+")")
    print(result_df)
    with pd.ExcelWriter(r"C:\Users\momas\OneDrive\Desktop\Thesis\Thesis\CODES\Revised Codes\Results\M0\M0.xlsx", engine="openpyxl",mode='a',if_sheet_exists='overlay') as writer:
            
            result_df.to_excel(writer, sheet_name='AllFeaturesAtOnce',startcol=3*(fold_num-1),index=False)
    
    y_prob[train_index] , y_prob[test_index] = yPR_tr , yPR_tst
    if recurrent !=0:
        y_prob[train_index] , y_prob[test_index] = yPR_tr2 , yPR_tst2
...
#    y_train_auxf, y_test_auxf = new_y[train_index], new_y[test_index]
    ...
    for i in test_index:
       if new_y[i] == 1:
            ...
            if i!=test_index[-2]:
                y_auxfill2[i+2] = 1
    #y_pred_matrix_whole = np.zeros((Sample_size,len(X.columns)*len(model_list)))
    ...
    plt.plot(t, y_prob , label='Probability')
             #, linestyle='dashed', linewidth = 3,
         #marker='o', markerfacecolor='blue', 
         #        markersize=12)
        #plt.plot(t, final_votes[:,mod], label='Vote'+str(model_list[mod]))
    #plt.plot(t, y_pred_matrix_whole, label='Prediction')
    ...
    # naming the y axis
    plt.ylabel('Probability & Prediction',fontsize=20)
    # giving a title to my graphco
    plt.title('Prediction of Ensemble with all features given at Once'+' (clf: '+str(Ensemble1)+',WS:'+str(window_size)+')',fontsize=22)
    ...
    #plt.savefig('./figs/Ensemble_mclfs_'+str(Ensemble1)+'_fold'+str(fold_num)+'.png', bbox_inches='tight')
    
    plt.show()
    #plt.savefig('./figs/Ensemble_mclfs_'+str(Ensemble1)+'_fold'+str(fold_num)+'.pgf', bbox_inches='tight')
    plt.figure(figsize=(18, 10))
    #ax = plt.figure.add_axes([0, 0, 1, 1])
    plt.xticks(fontsize=17,rotation=50)
    plt.yticks(fontsize=17)
           #y_final[train_index] , y_final[test_index] = y_pred_train, y_pred
    plt.plot(t_np[y_auxfill2_idx], y_prob[y_auxfill2_idx], label='Prob '+str(model_list[Ensemble1]))
             #, linestyle='dashed', linewidth = 3,
         #marker='o', markerfacecolor='blue', 
         #        markersize=12)
        #plt.plot(t, final_votes[:,mod], label='Vote'+str(model_list[mod]))
    ...
    # giving a title to my graphco
    plt.title('Prediction of Classifiers On Test Set'+' (#test fold:'+str(fold_num)+')',fontsize=22)
    plt.gca().spines["top"].set_alpha(.0)
    plt.gca().spines["bottom"].set_alpha(.3)
    plt.gca().spines["right"].set_alpha(.0)
    plt.gca().spines["left"].set_alpha(.3)
    ...
    plt.show()
#####Selecting the i best features:
#from sklearn.metrics import fbeta_score
#AUC_featureSelection = np.zeros((3,n))
#Fbet = np.zeros((3,n))
#for i in range(n):
    #if original == 0:
        #Xsubset = X01_Selected[:,n-i-1:]
    #else:
    #    Xsubset = np.concatenate((X01_Selected[:,n-i-1:],X_Selected[:,(window_size+M_Totsize)*(n-i-1):]),axis=1)
    
    #X_train, X_test, y_train, y_test = train_test_split(Xsubset, y_auxf,
                                                    #test_size=test_percent, random_state=42,
#                                                    shuffle=False)
    #meta_clf2.fit(X_train, y_train)
    #y_pred = meta_clf2.predict(X_test)
    #y_pred_train = meta_clf2.predict(X_train)
    #AUC_featureSelection[1,i] = roc_auc_score(y_train, y_pred_train)
    #Fbet[1,i]=fbeta_score(y_train, y_pred_train, average='binary', beta=3)    
    #AUC_featureSelection[2,i] = roc_auc_score(y_test, y_pred)
    #Fbet[2,i]=fbeta_score(y_test, y_pred, average='binary', beta=3)    
    #AUC_featureSelection[0,i] = i+1
    #Fbet[0,i]=i+1

#plt.figure(figsize=(10, 5))
#plt.plot(Fbet[0,:],Fbet[1,:],label="F3 training set")
#plt.plot(Fbet[0,:],Fbet[2,:],label="F3 test set")
#plt.plot(AUC_featureSelection[0,:],AUC_featureSelection[1,:],label="AUC training set")
#plt.plot(AUC_featureSelection[0,:],AUC_featureSelection[2,:],label="AUC test set")
#plt.xlabel('number of top selected features')
#plt.ylabel('F3/AUC score')
#plt.title('Feature Selection Optimization(WS:'+str(window_size)
          #+',Clf2:'+str(model_list[Ensemble2%10])+')')
